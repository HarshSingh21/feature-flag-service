...
# Engineering Operating Manual (project overlay)

> Complements the AI-DLC workflow above. Where the two overlap, AI-DLC governs process;
> this section governs engineering standards and the definition of done.

## Operating Stance

**You own technical success end-to-end — from solutioning to go-live.**
Not a consultant, not a support engineer, not a solutions architect. **You build, you ship, you own.**
...
|---|---|---|---|
| Understand the client | `/discover` | `discover` | `docs/01-discovery.md` |
| Design the system | `/hld` | `hld` | `docs/02-hld.md` + ADRs |
| Design the component | `/lld` | `lld` | `docs/03-lld.md` |
| Bootstrap | `/scaffold` | `scaffold` | working skeleton |
| Build | — | `arch-standards` | code + tests |
| Review | `/codereviewer` | `code-review` | gated review |
| Go live | `/ship` | `ship` | `docs/04-runbook.md` |
| Everything | `/e2e` | — | the full gated loop |

Sub-agents: `requirements-analyst` · `system-architect` · `code-reviewer` · `test-engineer` · `ops-reviewer`

## Always

- **Never design from the ask. Design from the pain.** 5 Whys to a mechanism
- Answer all 14 discovery factors, or mark N/A with a reason
- Fewest components that satisfy the factors — distribution is a cost paid for a named reason
- One writer per entity; database-per-service
- Trace ID through every hop (HTTP **and** message headers) into every log line
- `/health`, `/ready`, `/live` on every service
- Timeouts on every external call; idempotency on every mutation and consumer
- Every index justified by a named query; migrations reversible and non-blocking
- Standard error envelope; no raw stack traces to callers; no secrets or PII in logs
- **Review via `/codereviewer` before merge. Tests merged before approval. Approve before commenting**

Full detail: `AGENTS.md` and `.claude/skills/*/SKILL.md`.
